	...
	if (Event.reportObject.cmd == GENIE_REPORT_EVENT)
	{
		...
		if (Event.reportObject.object == GENIE_OBJ_USERBUTTON) //Userbuttons to select GCODE from SD
		{
			if (card.sdprinting || card.sdispaused){
				...
				else if ((Event.reportObject.index == BUTTON_SD_SELECTED ||  Event.reportObject.index == STRING_NAME_FILE) && surf_file_menu)
				{
					if(card.cardOK)
					{
				...
				else if ((Event.reportObject.index == BUTTON_SD_SELECTED1 ||  Event.reportObject.index == STRING_NAME_FILE) && surf_file_menu)
				{
					if(card.cardOK)
					{
						...
						dobloking = true;
						if(filepointer == 0){
							filepointer = fileCnt - 1;
						}
						else if((-1 + filepointer) > (fileCnt -1)){
							filepointer = 0;
						}
						else{
							filepointer -= 1;
				...
				else if ((Event.reportObject.index == BUTTON_SD_SELECTED2 ||  Event.reportObject.index == STRING_NAME_FILE2) && surf_file_menu)
				{
					if(card.cardOK)
					{
						card.getfilename(filepointer);
				...
				else if ((Event.reportObject.index == BUTTON_SD_SELECTED3 ||  Event.reportObject.index == STRING_NAME_FILE3) && surf_file_menu)
				{
					if(card.cardOK)
					{
						uint16_t fileCnt = card.getnrfilenames();
				...
				else if ((Event.reportObject.index == BUTTON_SD_LEFT || Event.reportObject.index == BUTTON_SD_RIGHT) && surf_file_menu) //TODO: control if SD is out
				{
					...
					if (card.cardOK){
						...
						if (card.filenameIsDir)
						{
							...
							//genie.WriteObject(GENIE_OBJ_USERIMAGES,0,1);
								...
								genie.WriteStr(STRING_NAME_FILE_DUR,"-d -h -m");//Printing form
								
								
								genie.WriteStr(STRING_NAME_FILE2_DUR,"-d -h -m");//Printing form
								
								
								genie.WriteStr(STRING_NAME_FILE3_DUR,"-d -h -m");//Printing form
								sdinitbool = true;
